...
def correctness1(client_params):
    if SCORES[client_params.client_key] >0 and client_params.params != None:
        return True, client_params.params
    else:
        return False, None
...
def update_model1(list_of_params, global_params):
    global ITERATION
    global ALL_PARAMS    
    # Update ITERATION
    ITERATION += 1
    # Apply Gradients and Update CENTRAL MODEL
    F_MODEL = modman.FederatedAveragingModel(list_of_params, global_params)
    return F_MODEL
...
def post_params():
    ...
    if len(ALL_PARAMS)==N_CLIENTS or U_TIME_STAMP<datetime.now():   # U_TIME_STAMP<datetime.now() or
        ...
        list_of_params =   collect_params1() # collect_params1() for only params and collectparams() for params and score
        if(len(list_of_params)>0): 
            data.append([f'\nUpdating global parameters with params from {len(list_of_params)} client.'])
            set_model = update_model1(list_of_params, CENTRAL_MODEL)
